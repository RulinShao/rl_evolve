...
export SLIME_DIR="/checkpoint/comem/rulin/rl_evolve"  # or wherever your rl_evolve repo is
...
# KEY CHANGE: Use local Megatron-LM path instead of /root/Megatron-LM
if [ ! -d "$SAVE_SHM_DIR/${MODEL_NAME}_torch_dist" ] || [ $FORCE_DOWNLOAD -eq 1 ]; then
    echo "Converting HF model to torch dist format..."
    PYTHONPATH=/checkpoint/comem/rulin/rl_evolve:$MEGATRON_DIR python tools/convert_hf_to_torch_dist.py ${MODEL_ARGS[@]} --hf-checkpoint $SAVE_SHM_DIR/$MODEL_NAME --save $SAVE_SHM_DIR/${MODEL_NAME}_torch_dist
